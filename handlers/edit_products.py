...
async def send_all_products(call: types.CallbackQuery):
    ...
    if products:
        for product in products:
            ...
            edit_button = InlineKeyboardButton('Редактировать', callback_data=f'edit_{product["productid"]}')
...
async def edit_products(call: types.CallbackQuery, state: FSMContext):
    ...
    name_button = InlineKeyboardButton(text="Название", callback_data="field_name")
    ...
    info_button = InlineKeyboardButton(text="Инфо ", callback_data="field_infoproduct")
    coll_button = InlineKeyboardButton(text="Коллекция", callback_data="field_collection")
    keyboard.add(name_button, category_button, price_button, size_button, photo_button, info_button, coll_button)
